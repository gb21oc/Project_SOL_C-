...
namespace WebApp.Identity
{
    public class MyUserStore : IUserStore<MyUser>, IUserPasswordStore<MyUser>
    {
        public async Task<IdentityResult> CreateAsync(MyUser user, CancellationToken cancellationToken)
        {
            using (var connection = GetOpenConnection())
            {
                string sql = 
                    ...
                    "[NormalizedUserName]," +
                    "[PasswordHash]) " +
                    "Values(" + "'" + user.Id + "'," + "'" + user.UserName + "'," + "'" + user.NormalizedUserName + "'," + "'" + user.PasswordHash + "')";
                    //new
                    //{
                    //    id = user.Id,
        ...
        {
            var connection = new SqlConnection(@"Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=IdentityCurso;Data Source=WKT-0065D\SQLEXPRESS");
